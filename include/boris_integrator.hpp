#ifndef _BORIS_INTEGRATOR_HPP
#define _BORIS_INTEGRATOR_HPP
/* Boris integrator
 *
 * integrator for a particle in magnetic field
 */
...
namespace {
    ...
    using cal_force_type = std::vector<double>(*)(const std::vector<double>&);

    // function type to calculate magnetic field
    //  all constant should be included, i.e. F_mag = cross(p, w)
    using cal_w_type = std::vector<double>(*)(const std::vector<double>&);

    void boris_integrator(  std::vector<double>& r, std::vector<double>& p, 
                            ...
                            cal_force_type cal_force, cal_w_type cal_w) 
    {
        ...
        r += half_dt / m * p;
        std::vector<double> F = cal_force(r);
        p += half_dt * F;

        // p -> R(0.5*dt*omega) * p
        //  R(z) = (I + z) * inv(I - z)
        std::vector<double> w = cal_w(r) * half_dt;
