...
namespace matrixop {
			...
			const char* jobz)
	{
		...
		DSYEVD(jobz, &CHARL, &N, &evt_[0], &N, &eva[0], 
			...
			int begin = 0, int len = -1)
	{
		...
		return DDOT(&N, &v1[begin], &ONEI, &v2[begin], &ONEI);
	}
			...
			int begin = 0, int len = -1)
	{
		...
		int N((len == -1) ? v1.size() : len);
		complex<double> rst;
		return ZDOTC(&N, &v1[begin], &ONEI, &v2[begin], &ONEI);
	}
	...
	inline vector<double> mlog(const vector<double>& Mat, int N) 
	{
		...
		lwork = -1;
		work.resize(1);
		ZGEES(&CHARV, &CHARN, nullptr, &N, &A[0], &N, &sdim, 
